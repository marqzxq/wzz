...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:wzz\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5511977782073:+5511977782073\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.' '*' '!'
blocked = []
limitawal = '999999999'
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				Iv: '𝙎𝙚𝙢 𝙡𝙞𝙣𝙠, 𝙙𝙚𝙨𝙘𝙪𝙡𝙥𝙚'
				},
				only: {
					group: '[❗] Este comando só pode ser usado em grupos! ❌',
					premium: '[❗] ESTE PEDIDO É SO PARA *USUÁRIOS PREMIUMS*',
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD WZZ BOT*',
					...
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, você não está registrado como meu amigo. Registre-se para utilizar o bot, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar nabuto|14\n\n──「 WZZ BOT 」──`,
...
axios.get(`https://www.receitaws.com.br/v1/cnpj/${aris}`).then((res) => {
	conn.sendMessage(id, '[❗] AGUARDE UM POUCO', MessageType.text)
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("que isso sirva de exemplo 🚶")
		}, 4000)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("2 segundos")
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundo KKKKKKKK adeus cria 👋🏻")
		}, 0)
	}
		...
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`tchau lixo ${sender.split("@")[0]} voce sera expulso deste grupo em 5 segundos`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 5000)
		setTimeout( () => {
		...
		if (messagesC.includes("bot")){
			...
			switch(command) {
				...
				case 'menu1':
					...
					if (args.length < 1) return reply('cade o texto filho da putaaaa')
					reply(mess.wait)
...
break
				...
				case 'wibu':
                   ...
                   break
					...
					break
				case 'desligar':
				if (!isOwner) return reply('Comando só para o meu dono, qm é vc?')
...
// recursos anime, random @nabuto domina vida
              ...
              case 'cerpen':
               ...
               case 'puisiimg':
                ...
                case 'bukagc':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					open = {
					text: `Grupo aberto pelo administrador @${sender.split("@")[0]}\nsekarang *todos os participantes* podem enviar mensagens`,
					contextInfo: { mentionedJid: [sender] }
					}
          ...
          reply('Espere um pouco.')
			    ...
			    case 'dono':
                    ...
                    client.sendMessage(from, 'Meu dono é esse lindo ai,salva contato la ',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT WZZ ✨* 」\n*│+ wa.me/5511977782073*╰──────*「 WZZ 」*────*\n\n*_SE QUER SER ADMIN DO BOT WZZ_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'bomdia':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'bom dia zap'})
					break
				case 'boatarde':
					memein = await kagApi.memeindo()
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* MARQUIN7\n*INSTA*: https://instagram.com/marquinwzz?igshid=15xcsjnk0u57b *WPP:* wa.me/+5511977782073\n*\n\nespero que tenham gostado do bot️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
